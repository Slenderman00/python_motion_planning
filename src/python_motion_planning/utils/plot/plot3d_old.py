...
class Plot3D:
    ...
    def plotEnv(self, name: str, draw_edge=False) -> None:
        ...
        self.ax3d.scatter(self.start.x, self.start.y, self.start.z, marker="s", color="#ff0000", label="start")
        self.ax3d.scatter(self.goal.x, self.goal.y, self.goal.z, marker="s", color="#1155cc", label="Goal")
        ...
        if isinstance(self.env, Grid3D):
            ...
            self.ax3d.scatter(obs_x, obs_y, obs_z, c="k", marker="s", label="Obstacles")

        if isinstance(self.env, Map3D):
            # boundary
            for (ox, oy, w, h) in self.env.boundary:
                self.ax3d.add_patch(patches.Rectangle(
                        (ox, oy), w, h,
                        edgecolor='black',
                        facecolor='black',
                        fill=True
            ...
            for (ox, oy, r) in self.env.obs_circ:
                self.ax3d.add_patch(patches.Circle(
                        (ox, oy), r,
                        edgecolor='black',
                        facecolor='gray',
                        fill=True
    ...
    def plotExpand(self, expand: list) -> None:
        """
        Plot expanded grids using in-graph searching on a 3D Axes (self.ax3d).
        ...
        if self.start in expands:
            expands.remove(self.start)
        if self.goal in expands:
            expands.remove(self.goal)

        fig = self.ax3d.figure
        # Single key handler for the whole plotting session
        ...
        # Grid3D: plot individual expanded cells/points as markers (with z)
        if isinstance(self.env, Grid3D):
            total = len(expands)
            for x in expands:
                count += 1
                # Draw a single square marker at (x, y, z)
                self.ax3d.plot(
                    [x.x], [x.y], [x.z],
                    linestyle='None', marker='s', color="#dddddd"
                )

                # Adaptive throttling like the original
                if count < total / 3:
                    length = 20
        ...
        # Map3D: plot edges from parent -> node in 3D
        if isinstance(self.env, Map3D):
            for x in expands:
                count += 1
                if getattr(x, "parent", None):
                    # Assume parent provides (px, py, pz) in logical way (already implemented)
                    px, py, pz = x.parent[0], x.parent[1], x.parent[2]
                    self.ax3d.plot(
